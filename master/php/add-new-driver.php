...
if(mysql_error()){
	...
	if(mysql_error()){
		...
		$time = date("H:i:s");
                ...
                if($attType == 3){
                    $status_cab = '0';
					$sqlCabOwner = mysql_query("UPDATE `cabs` SET `cab_owner` = '$dID' WHERE `cab_id` = '$cId'");
                }else{
                    $status_cab = '1';
                }
		...
		if(mysql_error()){
			...
			if($attType == 3){
				header("location: ../form/add-new-driver.php?cbId=".$cabId."&dt=4&owner=".$dID."&doj=".$jod);
			}else{
			header("location: ../form/starting-pay-amount.php?cbId=".$cabId."&dt=".$attType."&doj=".$jod."&owner=0");
			}
		  }
		 }
	}
