...
if(mysql_error()){
	...
	if(mysql_error()){
		...
		$date = date("Y-m-d");
		$time = date("H:i:s");
	$sql = mysql_query("INSERT INTO `assigned_cabs` (`cab_id`, `driver_id`, `assign_date`, `assign_time`, `status`) VALUES ('$cabId', '$dID', '$date', '$time', '$status')");
	
	     if(mysql_error()){
			echo 'ERROR: Please Assign cab for this Driver!!';
		 }else{
			   $eid = $_SESSION['EMP_ID'];
				$appType = "ADN";
