...
$passd = $_GET['pass'];
 ?> 
<form  name="frmCreateDriver" id="frmCreateDriver" method="post" action="../master/php/add-owner-driver.php" enctype="multipart/form-data" skip="true">
...
elseif($dt == 3){
	echo ' Owner Details';
	$ownerId = $_ROWS['driver_owner'];
	$sqlOwn = mysql_query("SELECT driver_name,driver_id from `driver` where driver_code='$ownerId'");
	if(!mysql_error()){
...
<input type="hidden"  name="ownId" value="<?php echo $owner; ?>" />
    &nbsp;</strong></td></tr>
  ...
  <tr class="row-inactive">
   <td>Another No.<em style="float:right;">+91-</em></td>
  ...
  <tr class="row-active">
    <td>Reference By.</td>
    <td><input name="ref" type="text" value="<?php echo $oid; ?>" /></td>
    <td>Pin</td>
  ...
  <tr class="row-inactive">
    ...
    <td><input type="text" name="jodadt" id="jodadt" value="<?php echo $_GET['doj']; ?>"  />
	  ...
	  <td><input name="pictures" type="file" class="text_input"  /></td>
  ...
  <tr class="row-active">
   ...
   <!-- <td>Insurance Exp. Date</td>
	...
	?>
    <tr class="row-inactive">
  <td colspan="4" align="right"><input name="submit" type="submit"  value="Save" class="NavButton" /> </td></tr>
</table>
</form>
<script language="javascript">
$("#frmCreateDriver").validate();
$('#frmCreateDriver').ajaxForm({
	type: "POST",
